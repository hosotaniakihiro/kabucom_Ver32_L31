...
# Version: Ver1.1-PRODUCTION-PUSH-SUMMARY-ENGINE-DIRECT-OHLC-FALLBACK
#          -PUSH-ONLY
#          -PIPELINE-FIRST
#          -IMPORT-COMPAT
#          -PREVIOUS-PUSH-PRESERVE
#          -DIRECT-OHLC-FALLBACK-WHEN-PIPELINE-EMPTY
# ------------------------------------------------------------
# ✔ PUSHベースのサマリー計算専用
# ✔ run_summary_pipeline を主経路として優先
# ✔ ranking 系ロジックは一切持たない
# ✔ PUSH merged summary は PUSH 専用で保持
# ✔ push_rows=0 時は previous PUSH merged summary を保持
# ✔ latest-only probe / maturity probe は PUSH 判定専用
# ✔ PUSH 可視化ログ強化
# ============================================================
...
def _safe_latest_dt(df: pd.DataFrame):
    ...
    for col in ("datetime", "end_time", "start_time", "snapshot_time", "received_at"):
...
def _normalize_push_source_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = _coalesce_numeric_columns(out, "volume", [
        "volume", "Volume", "TradingVolume", "trading_volume",
        "CumVolume", "cum_volume", "last_cum_volume",
    ])
    ...
    logger.info(
        "[PUSH SUMMARY ENGINE] normalized push source rows=%s cols=%s symbols=%s latest_dt=%s",
        len(out), len(out.columns), _safe_symbol_count(out), _safe_latest_dt(out),
    )
...
def _log_df_stats(label: str, df: pd.DataFrame) -> None:
    ...
    logger.info(
        "[PUSH SUMMARY STATS] %s | rows=%d cols=%d symbols=%d latest_dt=%s",
        label, rows, cols, symbols, latest_dt,
    )
...
def _call_summary_fn(fn: Callable, *, interval: int, label: str) -> pd.DataFrame:
    try:
        ...
        return _safe_copy_df(out)
    except TypeError:
        pass
    except Exception:
        logger.exception("[PUSH SUMMARY ENGINE] %s failed with interval kwarg", label)
        return pd.DataFrame()
    ...
    try:
        ...
        logger.exception("[PUSH SUMMARY ENGINE] %s failed with interval positional", label)
        return pd.DataFrame()
    ...
    try:
        ...
        logger.exception("[PUSH SUMMARY ENGINE] %s failed with legacy signature", label)
        return pd.DataFrame()
...
def _resolve_summary_source_df(interval: int) -> pd.DataFrame:
    tf = int(interval)
...
def _get_previous_push_merged_summary(interval: int) -> pd.DataFrame:
    tf = int(interval)
    df = _get_from_global_data([
        f"push_merged_summary_{tf}min",
        f"push_merged_summary_{tf}",
        f"merged_summary_{tf}min",
        f"merged_summary_{tf}",
        "merged_summary",
        f"push_summary_{tf}min",
        f"push_summary_{tf}",
        f"summary_{tf}min_df",
    ...
    if (not isinstance(df, pd.DataFrame)) or df.empty:
        ...
        if callable(getter):
            try:
                df = getter(tf, source="push")
                logger.info("[PUSH SUMMARY ENGINE] previous get_merged_summary(tf=%s, source=push) used", tf)
            except TypeError:
                try:
                    df = getter(tf)
                except Exception:
    ...
    out = _ensure_symbol(_ensure_datetime(out))
    logger.info(
        "[PUSH SUMMARY ENGINE] previous PUSH merged summary interval=%s rows=%s symbols=%s latest_dt=%s",
        tf, len(out), _safe_symbol_count(out), _safe_latest_dt(out),
    )
    return out
...
def _maturity_profile(df: pd.DataFrame) -> dict:
    ...
    try:
        out = df.copy()
        prof["rows"] = len(out)
        prof["symbols"] = _safe_symbol_count(out)

        if "technical_ready" in out.columns:
            prof["technical_ready_rows"] = int(pd.Series(out["technical_ready"]).fillna(False).astype(bool).sum())
        ...
        for col, key in (
            ("score", "score_nonzero"),
            ("score_buy", "score_buy_nonzero"),
            ("score_sell", "score_sell_nonzero"),
        ):
...
def _looks_immature(df: pd.DataFrame) -> tuple[bool, dict]:
    ...
    immature = (
        prof["rows"] > 0 and (
            (
                prof["technical_ready_rows"] == 0 and
                prof["hist_ge3"] == 0 and
                prof["hist_max"] <= 2.0
            )
            or
            (
                prof["score_nonzero"] == 0 and
                prof["score_buy_nonzero"] == 0 and
                prof["score_sell_nonzero"] == 0
            )
        )
    )
...
def _build_direct_ohlc_from_push_source(push_df: pd.DataFrame, *, interval: int) -> pd.DataFrame:
    """
    run_summary_pipeline が rows=0 を返した場合の最終救済。
    正規化済みPUSH tickから直接OHLC + 最低限のindicator/score列を作る。
    """
    ...
    if "datetime" not in ticks.columns or "symbol" not in ticks.columns or "close" not in ticks.columns:
        logger.warning(
            "[PUSH SUMMARY ENGINE][DIRECT OHLC] required cols missing interval=%s cols=%s",
            interval,
            list(ticks.columns),
        )
        return pd.DataFrame()
    ...
    logger.warning(
        "[PUSH SUMMARY ENGINE][DIRECT OHLC] built fallback interval=%s rows=%s symbols=%s latest_dt=%s",
        interval,
        len(out),
...
def _run_push_summary(interval: int) -> pd.DataFrame:
    ...
    summary_df = _resolve_summary_source_df(interval)
    push_df = _resolve_push_source_df()
    ...
    if module_name == "trading.summary.pipeline.summary_pipeline" and func_name == "run_summary_pipeline":
        try:
            ...
            if out.empty and not push_df.empty:
                logger.warning(
                    "[PUSH SUMMARY ENGINE] pipeline returned empty interval=%s push_rows=%s -> direct OHLC fallback",
                    interval,
                    len(push_df),
                )
                ...
                immature, prof = _looks_immature(out)
                logger.info(
                    "[PUSH SUMMARY ENGINE] direct fallback result interval=%s out_rows=%s latest_only=%s ratio=%.4f one_bar=%s total=%s immature=%s hist_max=%s score_nonzero=%s",
                    interval,
                    len(out),
                    latest_only,
                    ratio,
            ...
            logger.info(
                "[PUSH SUMMARY TRACE] interval=%s runner_result_rows=%s latest_dt=%s",
                interval,
                len(out),
                _safe_latest_dt(out),
            )
...
def _store_push_merged_summary(interval: int, df: pd.DataFrame) -> None:
    ...
    for name in (
        f"push_merged_summary_{interval}min",
        f"push_merged_summary_{interval}",
...
def build_summary(interval: int = 1) -> pd.DataFrame:
    ...
    previous_push_merged_df = _get_previous_push_merged_summary(interval)

    push_df = _run_push_summary(interval=interval)
    ...
    _log_df_stats("previous PUSH merged summary", previous_push_merged_df)
    ...
    if push_rows > 0:
        should_store = _should_store_push_summary(push_df, interval=interval)

        if should_store:
            _store_push_merged_summary(interval=interval, df=push_df)
            stored_new_push = True
        else:
            if isinstance(previous_push_merged_df, pd.DataFrame) and not previous_push_merged_df.empty:
                logger.warning(
                    "[PUSH SUMMARY ENGINE] preserve previous PUSH merged_summary because new push summary is not mature interval=%s push_rows=%s new_rows=%s prev_rows=%s",
                    interval,
                    push_rows,
                    len(push_df),
                    len(previous_push_merged_df),
                )
                _store_push_merged_summary(interval=interval, df=previous_push_merged_df)
            else:
                logger.warning(
                    "[PUSH SUMMARY ENGINE] overwrite skipped by maturity/latest-only guard interval=%s push_rows=%s",
                    interval,
                    push_rows,
                )
    else:
        if isinstance(previous_push_merged_df, pd.DataFrame) and not previous_push_merged_df.empty:
            logger.warning(
                "[PUSH SUMMARY ENGINE] keep previous PUSH merged_summary because push_rows=0 interval=%s prev_rows=%s",
                interval,
                len(previous_push_merged_df),
            )
            _store_push_merged_summary(interval=interval, df=previous_push_merged_df)
            ...
            logger.warning(
                "[PUSH SUMMARY ENGINE] skip PUSH merged_summary overwrite because push_rows=0 interval=%s",
                interval,
            )

    logger.info(
        "✅ push_summary_engine END interval=%s push_rows=%d stored_new_push=%s latest_dt=%s",
        interval,
        push_rows,
        stored_new_push,
        _safe_latest_dt(push_df),
    )
