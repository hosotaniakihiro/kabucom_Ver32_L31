...
# Version: Ver1.8-PRODUCTION-MODULAR-SCORING-CORE-MARKET-FILTER-PRIMARY
#          -LAZY-IMPORT-STABLE-SIGNATURE-FIX-COMPAT-PIPELINE
# ------------------------------------------------------------
# ✔ Ver1.7 完全保持
...
# ✔ FIX: run_scoring_pipeline の interval シグネチャ不一致に対応
# ✔ FIX: calculate_final_scores のシグネチャ差異にも安全対応
# ✔ FIX: scoring_pipeline.scoring_main を最優先で呼ぶ
# ✔ FIX: run_scoring_pipeline への過度な依存を除去
# ============================================================
...
def _lazy_import_scoring_pipeline_callable():
    """
    scoring pipeline の公開入口の実装差異を吸収する。
    優先順位:
      1) trading.scoring.core.scoring_pipeline.scoring_main
      2) trading.scoring.core.scoring_pipeline.run_scoring_pipeline
      3) trading.scoring.core.scoring_pipeline.run_pipeline
    """
    from trading.scoring.core import scoring_pipeline as sp

    candidates = [
        getattr(sp, "scoring_main", None),
        getattr(sp, "run_scoring_pipeline", None),
        getattr(sp, "run_pipeline", None),
    ]

    for fn in candidates:
        if callable(fn):
            logger.info(
                "[SCORING] resolved scoring pipeline callable -> %s.%s",
    ...
    raise ImportError(
        "No callable scoring pipeline entrypoint found in "
        "trading.scoring.core.scoring_pipeline "
        "(expected scoring_main / run_scoring_pipeline / run_pipeline)"
    )
...
def _call_scoring_pipeline(
    ...
    """
    scoring pipeline の実装差異を吸収する安全ラッパー。

    想定パターン:
      1) (df, interval=..., analysis_only=..., force=...)
      2) (df, interval=..., analysis_only=...)
      3) (df, interval=..., force=...)
      4) (df, analysis_only=..., force=...)
      5) (df, interval=...)
      6) (df, analysis_only=...)
      7) (df, force=...)
      8) (df)
    ...
    candidates = [
        {"interval": interval, "analysis_only": analysis_only, "force": force},
        {"interval": interval, "analysis_only": analysis_only},
        {"interval": interval, "force": force},
        {"analysis_only": analysis_only, "force": force},
        {"interval": interval},
    ...
    for kwargs in candidates:
        try:
            logger.debug(
                "[SCORING] scoring pipeline try kwargs=%s",
                kwargs,
            )
            return scoring_callable(df_out, **kwargs)
        except TypeError as e:
            call_errors.append(f"kwargs={kwargs} -> {e}")
            continue

    logger.error(
        "[SCORING] scoring pipeline signature mismatch: %s",
        " | ".join(call_errors),
    )
    raise TypeError("scoring pipeline signature mismatch")


def _call_calculate_final_scores(
    calculate_final_scores,
    df_out: pd.DataFrame,
    interval: str | int | None,
) -> pd.DataFrame:
    """
    calculate_final_scores の実装差異を吸収する安全ラッパー。
    想定パターン:
      1) (df, interval=...)
      2) (df)
    """
    ...
    for kwargs in candidates:
        try:
            logger.debug(
                "[SCORING] calculate_final_scores try kwargs=%s",
                kwargs,
            )
            return calculate_final_scores(df_out, **kwargs)
        except TypeError as e:
            call_errors.append(f"kwargs={kwargs} -> {e}")
            continue

    logger.error(
        "[SCORING] calculate_final_scores signature mismatch: %s",
        " | ".join(call_errors),
    )
    raise TypeError("calculate_final_scores signature mismatch")
...
def scoring_main(
    ...
    try:
        ...
        try:
            scoring_callable = _lazy_import_scoring_pipeline_callable()
            df_out = _call_scoring_pipeline(
            ...
            logger.exception("[SCORING] scoring pipeline execution failed")
            return pd.DataFrame()

        if df_out is None or df_out.empty:
            logger.info("[SCORING] empty after scoring pipeline")
            return pd.DataFrame()

        try:
            calculate_final_scores = _lazy_import_score_calculator()
            df_out = _call_calculate_final_scores(
