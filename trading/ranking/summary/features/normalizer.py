# ============================================================
# File   : trading/ranking/summary/features/normalizer.py
# Version: PRODUCTION-STABLE-REV1.0-RANKING-SUMMARY-NORMALIZER
# ============================================================
...
from database.schema.ranking_summary_schema import RANKING_SUMMARY_COLUMNS
...
def normalize_for_save(df: pd.DataFrame, *, interval: int) -> pd.DataFrame:
    ...
    if "symbol" not in out.columns:
        logger.warning("[RANKING SUMMARY NORMALIZER] no symbol column interval=%s", interval)
        return pd.DataFrame()

    if "datetime" not in out.columns:
        logger.warning("[RANKING SUMMARY NORMALIZER] no datetime column interval=%s", interval)
        return pd.DataFrame()

    ensure_col_from_alias(out, "symbolname", ["name", "symbol_name", "SymbolName"], "")

    ensure_col_from_alias(out, "current_price", ["close", "close_price", "price", "last_price", "CurrentPrice"], None)
    ensure_col_from_alias(out, "close", ["close_price", "current_price", "price", "last_price", "CurrentPrice"], None)
    ...
    fill_missing_from_alias(out, "current_price", ["close", "close_price"])
    fill_missing_from_alias(out, "close_price", ["close", "current_price"])
    ...
    ensure_col_from_alias(out, "volume", ["vol", "Volume", "volume_1m", "trading_volume"], 0)
    ...
    ensure_col_from_alias(out, "display_score", ["disp_score", "final_score", "score_total", "score"], None)
    ensure_col_from_alias(out, "disp_score", ["display_score", "final_score", "score_total", "score"], None)
    ensure_col_from_alias(out, "final_score", ["score_total", "display_score", "disp_score", "score"], None)
    ensure_col_from_alias(out, "score_total", ["final_score", "display_score", "disp_score", "score"], None)
    ...
    try:
        ...
        logger.warning(
            "[RANKING SUMMARY NORMALIZER] ranking feature calculation skipped interval=%s",
            interval,
            exc_info=True,
        )
    ...
    numeric_cols = [
        ...
        "rank", "best_rank", "hit_count", "hist",
        "change_percentage", "trading_volume", "trading_value", "turnover", "tick_count",
        ...
        "ranking_atr_proxy", "ranking_momentum", "rank_improve", "volume_delta", "ranking_score",
    ...
    if "price_source" in out.columns:
        out["price_source"] = out["price_source"].replace("", "ranking_snapshot")
    else:
    ...
    if before != after:
        logger.warning(
            "[RANKING SUMMARY NORMALIZER] input dedupe interval=%s before=%s after=%s deleted=%s",
            interval,
            before,
            after,
            before - after,
    ...
    if price_missing > 0:
        logger.warning(
            "[RANKING SUMMARY NORMALIZER] close missing rows interval=%s missing=%s total=%s",
            interval,
            int(price_missing),
            len(out),
        )
