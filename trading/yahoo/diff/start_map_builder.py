...
# Version: Ver1.1-PERIODIC-LOOKBACK-CAP
# ------------------------------------------------------------
# ✔ 定期: yahoo_1min latest ベース差分
# ✔ 定期: 取得開始を直近 N 分に丸める（既定45分）
...
def _periodic_lookback_floor(*, target_date: dt.date, end_dt: dt.datetime) -> dt.datetime:
    lookback_min = max(5, _env_int("YAHOO_PERIODIC_DOWNLOAD_LOOKBACK_MINUTES", _env_int("YAHOO_PERIODIC_REFLECT_LOOKBACK_MINUTES", 45)))
    floor = end_dt - dt.timedelta(minutes=lookback_min)
    session_start = dt.datetime.combine(target_date, DEFAULT_SESSION_START)
    if floor < session_start:
        floor = session_start
    return floor
...
def build_periodic_symbol_start_map(symbols: list[str], target_date: dt.date, end_dt: dt.datetime) -> dict[str, dt.datetime]:
    last_map = load_yahoo_1min_last_datetimes(target_date)
    default_start = dt.datetime.combine(target_date, DEFAULT_SESSION_START)
    lookback_floor = _periodic_lookback_floor(target_date=target_date, end_dt=end_dt)
    ...
    for sym in symbols:
        ...
        # 定期処理では新規銘柄でも朝9時まで戻さない。
        # 朝からの全履歴取得は startup 側に任せる。
        if start_dt < lookback_floor:
            start_dt = lookback_floor
            capped += 1
    ...
    try:
        starts = list(out.values())
        logger.warning(
            "[YAHOO START MAP] periodic built symbols=%s missing_latest=%s capped=%s start_min=%s start_max=%s end_dt=%s lookback_floor=%s lookback_min=%s sample=%s",
            len(out),
            missing_latest,
            capped,
            ...
            end_dt,
            lookback_floor,
            max(5, _env_int("YAHOO_PERIODIC_DOWNLOAD_LOOKBACK_MINUTES", _env_int("YAHOO_PERIODIC_REFLECT_LOOKBACK_MINUTES", 45))),
            list(out.items())[:10],
        )
    except Exception:
        pass
...
def _load_summary_last_datetimes(target_date: dt.date, interval: int) -> dict[str, dt.datetime]:
    db_path = _resolve_existing_summary_db_path(target_date)
    engine = None
    if not db_path:
        logger.info("[YAHOO START MAP] summary db not found target_date=%s interval=%s", target_date, interval)
        return {}
    ...
    try:
        ...
        logger.exception("[YAHOO START MAP] load summary latest failed interval=%s db=%s", interval, db_path)
        return {}
    finally:
        try:
            if engine is not None:
...
def _calc_start_from_summary_latest(*, latest_3m: dt.datetime | None, latest_5m: dt.datetime | None, today: dt.date, hard_floor: dt.datetime, end_dt: dt.datetime) -> dt.datetime:
    session_start = dt.datetime.combine(today, DEFAULT_SESSION_START)
    candidates: list[dt.datetime] = []

    if latest_3m is not None:
        candidates.append(latest_3m - dt.timedelta(minutes=SUMMARY_LOOKBACK_3MIN_MINUTES) - dt.timedelta(minutes=LOOKBACK_BUFFER_MINUTES))
    if latest_5m is not None:
        candidates.append(latest_5m - dt.timedelta(minutes=SUMMARY_LOOKBACK_5MIN_MINUTES) - dt.timedelta(minutes=LOOKBACK_BUFFER_MINUTES))

    start_dt = min(candidates) if candidates else hard_floor
...
def build_startup_symbol_start_map(symbols: list[str], target_date: dt.date, end_dt: dt.datetime) -> dict[str, dt.datetime]:
    ...
    for sym in symbols:
        ...
        start_dt = _calc_start_from_summary_latest(latest_3m=latest_3m, latest_5m=latest_5m, today=target_date, hard_floor=hard_floor, end_dt=end_dt)
...
__all__ = ["build_periodic_symbol_start_map", "build_startup_symbol_start_map"]
