...
# Version: Ver32-PRODUCTION-YAHOO-LOADER-BATCH-FIRST
# ------------------------------------------------------------
# ✔ Ver31 全機能保持
# ✔ Yahoo batch download 優先
# ✔ fallback で単一銘柄並列取得も維持
# ✔ scheduler安全
# ✔ runtime絶対停止しない
# ✔ dataframe構造保証
# ============================================================
...
from trading.yahoo.yahoo_batch_download_client import safe_download_batch
...
def _validate_loaded_df(df_all: pd.DataFrame) -> pd.DataFrame:
...
def load_multiple_symbols(
    ...
    *,
    ...
    """
    複数銘柄取得
    - まず batch download を試す
    - 空なら fallback で単一銘柄並列取得
    """
    ...
    logger.info(
        "[YAHOO DEBUG] load_multiple_symbols start symbols=%d interval=%s start=%s end=%s",
        len(symbols),
        interval,
        start_dt,
        end_dt,
    ...
    # 1) batch first
    # --------------------------------------------------------
    if len(symbols) >= BATCH_FETCH_MIN_SYMBOLS:
        try:
            df_batch = safe_download_batch(
                symbols,
                start_dt=start_dt,
                end_dt=end_dt,
                interval=interval,
            )
            df_batch = _validate_loaded_df(df_batch)

            if not df_batch.empty:
                logger.info(
                    "[YAHOO DEBUG] batch fetch success rows=%d symbols=%d range=%s -> %s",
                    len(df_batch),
            ...
            logger.warning("[YAHOO DEBUG] batch fetch empty -> fallback single-symbol parallel")
        except Exception:
            logger.exception("[YAHOO DEBUG] batch fetch failed -> fallback single-symbol parallel")

    # --------------------------------------------------------
    # 2) fallback single-symbol parallel
    # --------------------------------------------------------
    ...
    df_all = _validate_loaded_df(df_all)
    if df_all.empty:
        return pd.DataFrame()
