...
# Version: V1-ENTRY-EXECUTE-TIMEOUT-AND-STALE-GUARD
# ------------------------------------------------------------
# Purpose:
#   entry_controller._execute_best_candidate が長時間戻らず、
#   entry_controller_lock_timeout / pending滞留を起こす問題を防ぐ。
#
# Fix:
#   - 古い候補は元 executor に渡す前に拒否する。
#   - 元 executor の実行時間に上限を設ける。
#   - 同一 symbol/side の executor がまだ走っている場合は新規呼び出しを拒否する。
#   - 後段パッチで上書きされても watcher で再適用する。
# ============================================================
...
def _parse_dt(v: Any) -> dt.datetime | None:
    try:
        ...
        if v is None or str(v).strip() == "":
            return None
        s = str(v).strip()
        # pandas Timestamp string / ISO string
        try:
...
def _item_age_sec(item: dict[str, Any], row: dict[str, Any]) -> tuple[float | None, str]:
    ...
    ts = _first_dt(
        item.get("created_at"),
        item.get("pending_created_at"),
        ai.get("created_at") if isinstance(ai, dict) else None,
    )
    if ts is not None:
        return max(0.0, (dt.datetime.now() - ts).total_seconds()), "created_at"
    ts = _first_dt(
        item.get("datetime"),
        row.get("datetime"),
        item.get("entry_time"),
    ...
    if ts is not None:
        return max(0.0, (dt.datetime.now() - ts).total_seconds()), "datetime"
    return None, "missing"
...
def _candidate_stale_guard(item: dict[str, Any], row: dict[str, Any], symbol: str, side: str) -> bool:
    ...
    if age is None:
        return True
    # created_at はキュー投入時刻なので短め。datetime は足時刻なので少し長めに許容。
    ...
    logger.warning(
        "[ENTRY EXEC TIMEOUT GUARD] STALE_CANDIDATE_SKIP symbol=%s side=%s age=%.1fs max_age=%.1fs source=%s pruned=%s",
        symbol,
        side,
        age,
        max_age,
...
def _call_with_timeout(orig: Callable[..., Any], item: dict[str, Any], boost_active: bool, symbol: str, side: str) -> bool:
    ...
    def runner() -> None:
        try:
            ret = bool(orig(item, boost_active))
            try:
                q.put_nowait(("ok", ret))
            except Exception:
                pass
        except Exception as exc:  # keep exception away from scheduler/entry lock
            try:
    ...
    with _INFLIGHT_LOCK:
        ...
        if isinstance(existing, dict) and not bool(existing.get("done")):
            ...
            logger.warning(
                "[ENTRY EXEC TIMEOUT GUARD] INFLIGHT_DUPLICATE_SKIP symbol=%s side=%s age=%.1fs timeout=%.1fs",
                symbol,
                side,
                age,
                timeout,
    ...
    if th.is_alive():
        ...
        logger.warning(
            "[ENTRY EXEC TIMEOUT GUARD] ORIG_TIMEOUT_RETURN_FALSE symbol=%s side=%s timeout=%.1fs pruned=%s thread=%s",
            symbol,
            side,
            timeout,
            removed,
...
def _wrap_current() -> bool:
    try:
        ...
        if not callable(cur):
            return False
        if getattr(cur, "_entry_execute_timeout_guard_v1", False):
            return True
        orig = cur
        ...
        def wrapped(item: dict, boost_active: bool) -> bool:
            try:
                ...
                logger.warning("[ENTRY EXEC TIMEOUT GUARD] DONE symbol=%s side=%s ok=%s elapsed=%.3fs", symbol, side, ok, elapsed)
                return bool(ok)
            except Exception:
        ...
        logger.warning(
            "[ENTRY EXEC TIMEOUT GUARD] wrapped target=%s timeout=%.1fs stale_created=%.1fs stale_bar=%.1fs",
...
def install() -> bool:
    ...
    logger.warning("[ENTRY EXEC TIMEOUT GUARD] installed V1 ok=%s", ok)
    return bool(ok)
