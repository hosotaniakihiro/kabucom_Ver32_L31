...
Fixes two production failure modes observed in 2026-06-15 logs:
...
1. kabusapi register/unregister_all returns Code=4001009 / APIキー不一致.
   ...
   currently resolved X-API-KEY and returned False.  This patch retries once
   after token_manager.refresh_token(), using the freshly returned token for
   the retry request.  It also updates common env/global slots so subsequent
   calls resolve the same token.
...
VERSION = "V1-PUSH-REGISTER-AUTH-RETRY-AND-TARGET-TOPUP"
_INSTALLED = False
...
def _patch_register_ops() -> bool:
    ...
    def _http_json_request_patched(*, url: str, method: str, payload: Any, api_key: str, timeout: float = 10.0):
        ok, content = orig_http(url=url, method=method, payload=payload, api_key=api_key, timeout=timeout)
    ...
    try:
        ...
        logger.warning("[PUSH REGISTER RECOVERY] register_ops auth retry patched")
        return True
    except Exception:
...
def install() -> bool:
    ...
    logger.warning(
        "[PUSH REGISTER RECOVERY] installed version=%s register_retry=%s target_topup=%s min_keep=%s",
        VERSION,
        ok_register,
        ok_symbols,
