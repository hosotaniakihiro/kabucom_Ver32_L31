...
VERSION = "V1-SUMMARY-AI-1M-LOW-MOVE-RELAX"
_INSTALLED = False
_ORIGINAL = None
...
def _range_values(row: dict) -> tuple[float, float, float, float, float]:
    ...
    if rp > 1.0:
        rp = rp / 100.0
    return close, high, low, rv, rp


def _install_entry_order_low_move_relax() -> bool:
    global _ORIGINAL
    try:
        ...
        if not callable(cur):
            logger.warning("[SUMMARY AI 1M LOW MOVE RELAX] target not callable version=%s", VERSION)
            return False
        if getattr(cur, "_summary_ai_1m_low_move_relax_v1", False):
            return True
        ...
        def _patched_low_move_hard_block(entry_row: dict, *, symbol: str, source: str):
            ...
            try:
                ...
                close, high, low, rv, rp = _range_values(row)
                ...
                try:
                    ...
                    row["summary_ai_1m_low_move_relaxed"] = True
                    ...
                    logger.warning(
                        "[SUMMARY AI 1M LOW MOVE RELAX] retry symbol=%s source=%s range_value=%.4f range_pct=%.6f min_pct=%.6f ok=%s reason=%s version=%s",
                ...
                logger.exception("[SUMMARY AI 1M LOW MOVE RELAX] retry failed symbol=%s source=%s version=%s", symbol, source, VERSION)
                return result

        _patched_low_move_hard_block._summary_ai_1m_low_move_relax_v1 = True  # type: ignore[attr-defined]
        ...
        logger.warning("[SUMMARY AI 1M LOW MOVE RELAX] installed version=%s min_pct=%s min_value=%s", VERSION, os.getenv("SUMMARY_AI_1M_MIN_RANGE_PCT", "0.0003"), os.getenv("SUMMARY_AI_1M_MIN_RANGE_VALUE", "1.0"))
        return True
    except Exception:
        logger.exception("[SUMMARY AI 1M LOW MOVE RELAX] install failed version=%s", VERSION)
        return False
...
def install() -> bool:
    ...
    _INSTALLED = bool(_install_entry_order_low_move_relax())
    return _INSTALLED
...
try:
    ...
    logger.exception("[SUMMARY AI 1M LOW MOVE RELAX] auto install failed")
