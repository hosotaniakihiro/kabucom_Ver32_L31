...
# Version: V1-LOW-VOLATILITY-FINAL-ENTRY-GUARD
# ------------------------------------------------------------
# Purpose:
#   Prevent entries into stocks that have liquidity but almost no
#   intraday movement.  This guard is intentionally final-stage and
#   evidence-based:
#     - Missing volatility data does NOT block by itself.
#     - If range/ATR/change data exists and all available movement
#       measures are below threshold, the entry is rejected.
#     - Applies to SUMMARY AI final selection and AI.entry_gate so it
#       catches PUSH, RANKING, and TONOSAMA-like rows that pass earlier
#       score/liquidity rescues.
# ============================================================
...
def _low_vol_block(row: Any) -> tuple[bool, str, dict[str, Any]]:
    ...
    # Do not block solely because volatility data is absent during startup.
    if len(evidence) < min_evidence:
        return False, "", {"evidence_count": len(evidence)}
...
def _block_result(row: Any, original: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    logger.warning(
        "[LOW VOL ENTRY GUARD] blocked symbol=%s side=%s detail=%s",
        detail.get("symbol"),
        detail.get("side"),
        detail,
    )
    return {
        ...
        "lot_multiplier": 0.0,
        "reason": reason,
        "model_used": "LOW_VOLATILITY_GUARD",
        "detail": detail,
    }
...
def _patch_ai_entry_gate() -> bool:
    try:
        ...
        if not callable(cur):
            return False
        if getattr(cur, "_low_volatility_guard_v1", False):
            return True
        orig = cur
        ...
        def patched_ai_final_entry_check(row: dict) -> dict:
            result = orig(row)
            try:
                # Keep existing reject reasons, but veto any allowed low-volatility row.
        ...
        logger.warning("[LOW VOL ENTRY GUARD] AI.entry_gate patched")
        return True
    except Exception:
    ...
    try:
        ...
        def patched_filter_blocked_ai_ok_items(ok_items):
            ...
            try:
                ...
                if skipped:
                    logger.warning(
                        "[LOW VOL ENTRY GUARD] SUMMARY_AI prefilter before=%s after=%s skipped=%s",
                        len(ok_items or []),
                        len(kept),
                        skipped[:50],
                    )
        ...
        logger.warning("[LOW VOL ENTRY GUARD] SUMMARY_AI executor prefilter patched")
        return True
    except Exception:
