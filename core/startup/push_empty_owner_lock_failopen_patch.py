...
# Version: V1.1-PUSH-EMPTY-OWNER-LOCK-FAILOPEN-MAIN-OR-RECEIVER
# ------------------------------------------------------------
# 目的:
#   PUSH single-owner lock が OS 的には掴まれているのに、lock file の
#   owner text が空で owner_pid も読めない場合、PUSH受信プロセスが
#   connected=False / total_received=0 で待ち続けるのを防ぐ。
#
# 方針:
#   main.py または push_receiver_runner.py だけ、空owner lock で一定秒数
#   待った後は lock_handle=None の fail-open で WebSocket 起動へ進める。
#   ranking_collector / yahoo_complement / summary_database / db_prepare は対象外。
# ============================================================
...
def _is_push_receiver_context() -> bool:
    argv = _argv_text()
    return "push_receiver_runner.py" in argv or os.getenv("AUTOSTOCK_PUSH_RECEIVER_PROCESS") == "1"


def _is_allowed_owner_context() -> bool:
    return _is_main_py_context() or _is_push_receiver_context()


def _is_excluded_context() -> bool:
    try:
        argv = _argv_text()
        # push_receiver は明示的に許可する。
...
def install() -> bool:
    ...
    if not _env_bool("PUSH_STREAM_EMPTY_OWNER_FAILOPEN", True):
        ...
        return False
    if _is_excluded_context() or not _is_allowed_owner_context():
        logger.warning(
            "[PUSH EMPTY OWNER FAILOPEN] skipped context main_py=%s push_receiver=%s excluded=%s argv=%s",
            _is_main_py_context(),
    ...
    if not callable(old_wait):
        ...
        return False
    if getattr(old_wait, "_push_empty_owner_failopen_v11", False):
        _INSTALLED = True
        return True
        ...
        while not state._stop_event.is_set():
            ...
            if _is_empty_unknown_owner_detail(detail):
                ...
                if elapsed_empty >= failopen_sec:
                    ...
                    logger.warning(
                        "[PUSH EMPTY OWNER FAILOPEN] fail-open after %.1fs context main_py=%s push_receiver=%s detail=%s argv=%s",
                        elapsed_empty,
                        _is_main_py_context(),
                        _is_push_receiver_context(),
                        detail,
    ...
    _wait_for_single_owner_lock_failopen._push_empty_owner_failopen_v11 = True  # type: ignore[attr-defined]
    ...
    _INSTALLED = True
    logger.warning(
        "[PUSH EMPTY OWNER FAILOPEN] installed v1.1 main_py=%s push_receiver=%s failopen_sec=%s argv=%s",
        _is_main_py_context(),
        _is_push_receiver_context(),
