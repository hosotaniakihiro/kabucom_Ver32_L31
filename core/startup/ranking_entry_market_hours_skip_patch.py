...
def _patch_once():
    try:
        ...
        if getattr(cur, '_ranking_market_hours_skip_v1', False): return True
        orig = cur
        def patched():
            now = dt.datetime.now()
            if not _in_session(now):
                ...
                return 0
            return orig()
        patched._ranking_market_hours_skip_v1=True
        patched._original=orig
        tasks._run_ranking_entry_safe=patched
        logger.warning('[RANKING ENTRY MARKET HOURS SKIP] patched _run_ranking_entry_safe')
        return True
    except Exception:
...
def install():
    ...
    ok=_patch_once(); threading.Thread(target=_watch,name='ranking-entry-market-hours-skip',daemon=True).start(); _DONE=True
