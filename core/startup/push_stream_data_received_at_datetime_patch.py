...
VERSION = "REV2-PUSH-STREAM-DATA-RECEIVED-AT-DATETIME-SAFE"
_INSTALLED = False
...
def _looks_like_escaped_garbage(value: Any) -> bool:
    """Return True for accidental repr/escaped blobs, not normal datetimes.

    The previous REV1 patch accepted any non-empty received_at-like value when
    datetime parsing failed.  If a bad caller accidentally passed a repr string
    containing thousands of backslashes, that garbage became stream_data.datetime
    and then polluted logs/DB output.  Only real datetime-like strings should be
    allowed through this patch.
    """
    try:
        s = str(value)
    except Exception:
    ...
    if not s:
        return True
    # ISO timestamps are short.  Long escaped payloads are never valid here.
    if len(s) > 80:
        return True
    # UNC paths contain a few backslashes; a received_at datetime should contain none.
    if s.count("\\") >= 2:
        return True
    # repr/list/dict fragments should never be used as datetime.
        ...
        def patched(self: Any, row: dict) -> Any:
            if _env_on("PUSH_STREAM_DATA_USE_RECEIVED_AT_DATETIME", True):
                try:
                    ...
                    if received_at and not _looks_like_escaped_garbage(received_at):
                        ...
                        logger.warning(
                            "[PUSH STREAM DATA RECEIVED_AT PATCH] ignored suspicious received_at symbol=%s len=%s backslashes=%s",
                            row.get("symbol"),
