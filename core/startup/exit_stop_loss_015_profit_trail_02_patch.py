...
def _patched_normal(pos: Any, price: float, now: dt.datetime):
    pnl = _pnl(pos, price)
    stop = _env_pct("ABSOLUTE_ENTRY_STOP_LOSS_PCT", 0.15)
    if pnl <= -stop:
        logger.warning("[EXIT STOP 0.15] symbol=%s side=%s pnl=%.4f%% price=%s entry=%s", _symbol(pos), _side(pos), pnl * 100.0, price, _entry(pos))
        return "NORMAL_STOP_0P15"
    high, low, mfe, retrace = _update_extreme(pos, price)
    trail_start = _env_pct("EXIT_PROFIT_TRAIL_START_PCT", 0.20)
    trail_dd = _env_pct("EXIT_PROFIT_TRAIL_DRAWDOWN_PCT", 0.20)
    if mfe >= trail_start and retrace >= trail_dd:
        logger.warning("[EXIT PROFIT TRAIL 0.2] symbol=%s side=%s mfe=%.4f%% retrace=%.4f%% high=%s low=%s price=%s", _symbol(pos), _side(pos), mfe * 100.0, retrace * 100.0, high, low, price)
        return "NORMAL_PROFIT_TRAIL_0P2"
    try:
...
def _patched_tonosama(pos: Any, price: float, now: dt.datetime):
    pnl = _pnl(pos, price)
    stop = _env_pct("TONOSAMA_STOP_LOSS_PCT", 0.20)
    if pnl <= -stop:
        logger.warning("[EXIT STOP TONOSAMA 0.20] symbol=%s side=%s pnl=%.4f%% price=%s entry=%s", _symbol(pos), _side(pos), pnl * 100.0, price, _entry(pos))
        return "TONOSAMA_STOP_0P20"
    high, low, mfe, retrace = _update_extreme(pos, price)
    trail_start = _env_pct("EXIT_PROFIT_TRAIL_START_PCT", 0.20)
    trail_dd = _env_pct("EXIT_PROFIT_TRAIL_DRAWDOWN_PCT", 0.20)
    if mfe >= trail_start and retrace >= trail_dd:
        logger.warning("[EXIT PROFIT TRAIL TONOSAMA 0.2] symbol=%s side=%s mfe=%.4f%% retrace=%.4f%% high=%s low=%s price=%s", _symbol(pos), _side(pos), mfe * 100.0, retrace * 100.0, high, low, price)
        return "TONOSAMA_PROFIT_TRAIL_0P2"
    try:
...
def install() -> bool:
    ...
    if _INSTALLED:
        return True
    try:
        os.environ["ABSOLUTE_ENTRY_STOP_LOSS_PCT"] = "0.15"
        os.environ["TONOSAMA_STOP_LOSS_PCT"] = "0.20"
        os.environ["EXIT_PROFIT_TRAIL_START_PCT"] = "0.20"
        os.environ["EXIT_PROFIT_TRAIL_DRAWDOWN_PCT"] = "0.20"
        ...
        if not callable(cur_n) or not callable(cur_t):
            ...
            return False
        if getattr(cur_n, "_exit_stop015_trail02_v1", False):
            _INSTALLED = True
            return True
        _ORIG_NORMAL = getattr(cur_n, "_original", cur_n)
        _ORIG_TONOSAMA = getattr(cur_t, "_original", cur_t)
        _patched_normal._exit_stop015_trail02_v1 = True  # type: ignore[attr-defined]
        _patched_normal._original = _ORIG_NORMAL  # type: ignore[attr-defined]
        _patched_tonosama._exit_stop015_trail02_v1 = True  # type: ignore[attr-defined]
        ...
        _INSTALLED = True
        logger.warning("[EXIT STOP/TRAIL PATCH] installed normal_stop=0.15%% tonosama_stop=0.20%% profit_trail=0.20%%")
        return True
    except Exception:
