...
# Version: V1-LAST-SIGNATURE-COMPAT
# ------------------------------------------------------------
# 目的:
...
def _make_adapter(orig):
    def _adapted_board_guard(row: Any, item: Any = None, symbol: Any = None, side: Any = None, *args: Any, **kwargs: Any) -> bool:
        ...
        try:
            ...
            # 3引数guardへ変換
            try:
                logger.warning("[FINAL BOARD GUARD SIGNATURE LAST] 4args failed -> retry 3args symbol=%s side=%s err=%s", sym, sd, e4)
                ...
                try:
                    ...
                    # 板欠落は別の board_missing_allow / final guard が判断済みなので、ここでは fail-open
                    return True
            except Exception:
...
def _patch_once() -> bool:
    ...
    try:
        ...
        if not callable(cur):
            return False
        if getattr(cur, "_final_board_guard_signature_last_v1", False):
            _INSTALLED = True
            return True
        _ORIG = cur
        ...
        _INSTALLED = True
        logger.warning("[FINAL BOARD GUARD SIGNATURE LAST] patched guard=%s", getattr(cur, "__name__", type(cur).__name__))
        return True
    except Exception:
...
def install() -> bool:
    ...
    logger.warning("[FINAL BOARD GUARD SIGNATURE LAST] installed ok=%s watcher=%s", ok, _WATCHER)
    return bool(ok)
