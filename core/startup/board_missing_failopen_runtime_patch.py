...
# Version: V1.7-DISABLED-SAFE-BOARD-REQUIRED
# ------------------------------------------------------------
...
def install() -> bool:
    ...
    _INSTALLED = True
    logger.warning(
        "[BOARD MISSING FAILOPEN] disabled safe install=%s guard_ok=%s order_safe=%s lock_retry_ok=%s stale_rescue_ok=%s hard_block=%s allow_without_board=%s summary_ai_allow=%s fast_boardless=%s version=%s",
        _INSTALLED,
        guard_ok,
        order_ok,
        lock_retry_ok,
