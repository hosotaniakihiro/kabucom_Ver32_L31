...
# Version: V1-MEMORY-1M-SCORE-MTF-ENRICH
# ------------------------------------------------------------
# 【目的】
...
def _all_zero(df: pd.DataFrame, cols: tuple[str, ...]) -> bool:
    nz = _nonnull_nonzero(df, cols)
    return all(v == 0 for v in nz.values())
...
def enrich_memory_1m_summary(df: pd.DataFrame, *, reason: str = "unknown") -> pd.DataFrame:
    ...
    try:
        ...
        before = _nonnull_nonzero(out, ("slope", "score", "score_buy", "score_sell", "mtf", "macd", "signal"))

        if "range_pct" not in out.columns or int((_num(out, "range_pct", 0.0).abs() > 1e-12).sum()) == 0:
            out["range_pct"] = range_pct
        if "atr" not in out.columns or int((_num(out, "atr", 0.0).abs() > 1e-12).sum()) == 0:
            out["atr"] = atr

        if "slope" not in out.columns or int((_num(out, "slope", 0.0).abs() > 1e-12).sum()) == 0:
            out["slope"] = fallback_slope
        if "slope_atr_scaled" not in out.columns or int((_num(out, "slope_atr_scaled", 0.0).abs() > 1e-12).sum()) == 0:
            out["slope_atr_scaled"] = fallback_slope
        if "score_slope" not in out.columns or int((_num(out, "score_slope", 0.0).abs() > 1e-12).sum()) == 0:
        ...
        pos_bias = pos_bias.where(~((slope.abs() < 1e-9) & (range_pct >= 0.0015) & (close_pos >= 0.65)), 1.0)
        pos_bias = pos_bias.where(~((slope.abs() < 1e-9) & (range_pct >= 0.0015) & (close_pos <= 0.35)), -1.0)

        directional = (slope * 1000.0).clip(-4.0, 4.0)
        directional = directional.where(directional.abs() > 1e-9, pos_bias)
        range_bonus = (range_pct * 30.0).clip(0.0, 2.5)
        ...
        if "score_buy" not in out.columns or int((_num(out, "score_buy", 0.0).abs() > 1e-12).sum()) == 0:
            out["score_buy"] = buy_score.fillna(0.0)
        if "score_sell" not in out.columns or int((_num(out, "score_sell", 0.0).abs() > 1e-12).sum()) == 0:
            out["score_sell"] = sell_score.fillna(0.0)

        score_signed = _num(out, "score_buy", 0.0) - _num(out, "score_sell", 0.0)
        if "score" not in out.columns or int((_num(out, "score", 0.0).abs() > 1e-12).sum()) == 0:
            out["score"] = score_signed
        if "score_total" not in out.columns or int((_num(out, "score_total", 0.0).abs() > 1e-12).sum()) == 0:
            out["score_total"] = score_signed
        if "final_score" not in out.columns or int((_num(out, "final_score", 0.0).abs() > 1e-12).sum()) == 0:
            out["final_score"] = score_signed
        if "display_score" not in out.columns or int((_num(out, "display_score", 0.0).abs() > 1e-12).sum()) == 0:
            out["display_score"] = score_signed

        if "mtf" not in out.columns or int((_num(out, "mtf", 0.0).abs() > 1e-12).sum()) == 0:
            out["mtf"] = score_signed.clip(-3.0, 3.0)
        if "score_mtf" not in out.columns or int((_num(out, "score_mtf", 0.0).abs() > 1e-12).sum()) == 0:
            out["score_mtf"] = score_signed.clip(-3.0, 3.0)
        if "mtf_score" not in out.columns or int((_num(out, "mtf_score", 0.0).abs() > 1e-12).sum()) == 0:
            out["mtf_score"] = score_signed.clip(-3.0, 3.0)

        # 1本だけで本物のMACDは成熟しないため、AI候補作成用に方向 proxy を入れる。
        if "macd" not in out.columns or int((_num(out, "macd", 0.0).abs() > 1e-12).sum()) == 0:
            out["macd"] = (slope * close).fillna(0.0)
        if "signal" not in out.columns or int((_num(out, "signal", 0.0).abs() > 1e-12).sum()) == 0:
            out["signal"] = 0.0
        if "hist" not in out.columns or int((_num(out, "hist", 0.0).abs() > 1e-12).sum()) == 0:
        ...
        after = _nonnull_nonzero(out, ("slope", "score", "score_buy", "score_sell", "mtf", "macd", "signal"))
        logger.warning(
...
def install() -> bool:
    ...
    try:
        ...
        if callable(cur_build) and not getattr(cur_build, "_summary_memory_1m_enrich_v1", False):
        ...
        if callable(cur_publish) and not getattr(cur_publish, "_summary_memory_1m_enrich_publish_v1", False):
