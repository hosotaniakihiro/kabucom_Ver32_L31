...
Also chains two summary-health fixes:
  * quarantine empty/old summary spool files so they are not retried forever
  * fill zero-volume recovered PUSH 3m/5m display rows before liquidity filtering
"""
...
VERSION = "V3-SUMMARY-LATEST-PREFER-SPOOL-MTF-HEALTH"
_INSTALLED = False
_ORIGINAL = None

_REALTIME_TIME_COLS = (
    "received_at", "recv_at", "recv_time", "last_recv", "last_recv_at", "last_received_at",
    "push_received_at", "push_time", "tick_time", "quote_time", "current_price_time",
    "CurrentPriceTime", "price_time", "event_time", "updated_at", "created_at",
...
def _floor_to_interval_min(s: pd.Series, interval: int) -> pd.Series:
    try:
        ...
        return s.dt.floor(f"{interval_i}min")
    except Exception:
        try:
...
def _promote_latest_datetime(latest: pd.DataFrame, interval: int, min_symbols: int) -> pd.DataFrame:
    try:
        if not _latest_is_usable(latest, min_symbols) or "datetime" not in latest.columns:
            return latest
        ...
        logger.warning(
            ...
            interval, best_col, int(mask.sum()), before, after, best_max, now,
        )
        return out
    except Exception:
...
def _install_chained_health_patches() -> bool:
    ok = False
    try:
        if os.environ.get("DISABLE_SUMMARY_SPOOL_QUARANTINE_PATCH", "").strip() != "1":
            from . import summary_spool_quarantine_patch
            ok = bool(summary_spool_quarantine_patch.install()) or ok
    except Exception:
        logger.exception("[SUMMARY LATEST PREFER PATCH] spool quarantine chain install failed")
    try:
        if os.environ.get("DISABLE_PUSH_MTF_VOLUME_FALLBACK_PATCH", "").strip() != "1":
            from . import push_mtf_volume_fallback_patch
            ok = bool(push_mtf_volume_fallback_patch.install()) or ok
    except Exception:
        logger.exception("[SUMMARY LATEST PREFER PATCH] push MTF volume fallback chain install failed")
    return ok
...
def install() -> bool:
    ...
    if _INSTALLED:
        return True
    try:
        os.environ.setdefault("SUMMARY_SPOOL_QUARANTINE_ENABLED", "1")
        os.environ.setdefault("SUMMARY_SPOOL_QUARANTINE_EMPTY", "1")
        os.environ.setdefault("SUMMARY_SPOOL_QUARANTINE_OLD", "1")
        os.environ.setdefault("SUMMARY_SPOOL_QUARANTINE_MAX_AGE_DAYS", "2")
        os.environ.setdefault("PUSH_MTF_VOLUME_FALLBACK_ENABLED", "1")
        os.environ.setdefault("PUSH_MTF_ZERO_VOLUME_FALLBACK_VOLUME", "30000")
            ...
            try:
                ...
                if force_latest:
                    ...
                    logger.warning(
                        ...
                        interval_i, reason, hist_dt, latest_dt, realtime_dt, delta_sec,
                        hist_symbols, latest_symbols,
                        len(payload) if isinstance(payload, pd.DataFrame) else 0,
                        _safe_latest_dt(payload), lag_sec,
                    )
                    return payload
                return _ORIGINAL(interval, df_hist, df_latest, normalize_fn)
            except Exception:
        ...
        health_ok = _install_chained_health_patches()
        _INSTALLED = True
        logger.warning(
            "[SUMMARY LATEST PREFER PATCH] installed version=%s intervals=%s lag_sec=%s wall_lag_sec=%s min_symbols=%s health=%s",
            VERSION,
