...
# Version: V1-MAIN-PUSH-DB-REFRESH-WHEN-MEMORY-STALE
# ------------------------------------------------------------
...
def _latest_dt(df: pd.DataFrame) -> Any:
...
def install() -> bool:
    ...
    try:
        ...
        if not callable(cur):
            logger.warning("[SUMMARY MAIN PUSH DB REFRESH] target loader unavailable version=%s", VERSION)
            return False
        if getattr(cur, "_summary_main_push_db_refresh_v1", False):
            _PATCHED = True
            return True
        ...
        _PATCHED = True
        logger.warning(
            "[SUMMARY MAIN PUSH DB REFRESH] installed enabled=%s max_stale=%.1f limit=%s version=%s",
