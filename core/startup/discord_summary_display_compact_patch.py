...
# Version: V1.0-COMPACT-DISCORD-SUMMARY-DISPLAY
# ------------------------------------------------------------
# 目的:
...
def _send_to_discord_compact(disp: Any, lines: list[str], title: str | None = None) -> None:
    try:
        ...
        # Discordの横崩れを避けるため、短い改行テキストとして送る。
        # code blockは日本語幅が環境でズレるので使わない。
        limit = 1800
        chunks: list[str] = []
        while text:
            if len(text) <= limit:
        ...
        for idx, chunk in enumerate(chunks, start=1):
            ...
            logger.info(
                "[DISCORD SUMMARY COMPACT] sent chunk=%s/%s ok=%s chars=%s",
                idx,
                len(chunks),
                ok,
                len(chunk),
    ...
    try:
        ...
        if callable(old_print_summary) and not getattr(old_print_summary, "_discord_1min_suppress_patch", False):
            def _print_summary_top10_patched(summary_df, interval_label="1min", *, notify_discord=True, **kwargs):
                if notify_discord and _is_1min_label(interval_label) and not _env_bool("SUMMARY_DISCORD_SEND_1MIN", False):
        ...
        if callable(old_print_ranking) and not getattr(old_print_ranking, "_discord_1min_suppress_patch", False):
            def _print_ranking_summary_top10_patched(summary_df, interval_label="1min", *, notify_discord=True, **kwargs):
                if notify_discord and _is_1min_label(interval_label) and not _env_bool("RANKING_SUMMARY_DISCORD_SEND_1MIN", False):
        ...
        _PATCHED = True
        logger.warning(
            "[DISCORD SUMMARY COMPACT] installed compact_display=True suppress_summary_1min=%s suppress_ranking_1min=%s",
            not _env_bool("SUMMARY_DISCORD_SEND_1MIN", False),
            not _env_bool("RANKING_SUMMARY_DISCORD_SEND_1MIN", False),
        )
        return True
    except Exception:
