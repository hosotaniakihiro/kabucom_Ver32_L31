...
def _all_zero(*xs:float)->bool:
    eps=abs(float(os.getenv('ENTRY_ORDER_SHORT_MTF_ZERO_EPS','0.0000001')))
    return all(abs(float(x or 0.0)) <= eps for x in xs)
...
def _can_skip_mtf_ng(row:dict, side:str, source:str, result:dict)->tuple[bool,dict]:
    ...
    # PUSH初期/補完直後は 1m/3m/5m slope が全部0.0になることがある。
    # これは反対方向ではなく未確定・中立なので、AI_OK + 流動性OKなら発注を止めない。
    if _b('ENTRY_ORDER_ALLOW_ALL_ZERO_SHORT_MTF', True) and _all_zero(s1,s3,s5):
        return True, {'reason':'all_zero_short_mtf_neutral_pass','side':side,'score':score,'slopes':{'slope_1m':s1,'slope_3m':s3,'slope_5m':s5},'original_reason':reason}
    if side=='SELL':
...
def install():
    ...
    try:
        ...
        if getattr(cur,'_short_mtf_neutral_direct_v2',False): _DONE=True; return True
        base=getattr(cur,'_original',cur)
        def patched(entry_row, *, symbol:str, side:str, source:str):
            res=base(entry_row, symbol=symbol, side=side, source=source)
            try:
                if res is not None:
            ...
            return res
        patched._short_mtf_neutral_direct_v2=True
        patched._short_mtf_neutral_direct_v1=True
        patched._original=base
        eob._summary_mtf_direction_guard=patched
        _DONE=True
        logger.warning('[ENTRY ORDER SHORT MTF NEUTRAL DIRECT] installed v2 allow_all_zero=%s', _b('ENTRY_ORDER_ALLOW_ALL_ZERO_SHORT_MTF', True))
        return True
    except Exception:
