...
# Version: v9-FUTURE-TIME-AND-PUSH-RECEIVE-GUARD
# ------------------------------------------------------------
# Purpose:
#   場中に古い/未来時刻/未受信PUSH summaryを使ってTONOSAMAを動かさない。
#
# Fix:
#   - latest が未来時刻なら age<0 でも fresh 扱いしない。
#   - PUSH WebSocket が connected=True でも total_received=0 / last_recv=None なら
#     TONOSAMAのPUSH由来エントリーを止める。
#   - summary_loader.load_merged_summary でも未来/古いsummaryをそのまま返さず、
#     短時間だけGC上の新鮮なsummaryを待つ。
#   - runtime.monitor_symbols が小さすぎる場合、global/dynamic providerで補完する。
# ============================================================
...
def _future_tolerance_sec() -> float:
    return max(0.0, _env_float("TONOSAMA_FUTURE_SUMMARY_TOLERANCE_SEC", 60.0))


def _is_future_summary_dt(latest: dt.datetime | None, now: dt.datetime | None = None) -> bool:
    if latest is None:
        return False
    now = now or dt.datetime.now()
    return latest.replace(tzinfo=None) > now.replace(tzinfo=None) + dt.timedelta(seconds=_future_tolerance_sec())
...
def _candidate_dfs() -> list[tuple[str, Any]]:
    ...
    try:
        ...
        if not getattr(load_merged_summary, "_tonosama_loader_wait_v9", False):
...
def _push_stream_receive_ok() -> bool:
    """PUSH接続済みでも未受信ならTONOSAMAを走らせないための軽量ヘルスチェック。"""
    if not _env_bool("TONOSAMA_REQUIRE_PUSH_STREAM_RECEIVE", True):
        return True
    try:
        ...
        last_recv = getattr(state, "_last_message_at", None)
        connected = bool(getattr(state, "_connected_event", None) and state._connected_event.is_set())
        ws_alive = bool(_is_ws_alive())
        if total_received <= 0 or last_recv is None:
            logger.warning(
                "[TONOSAMA ENTRY SCHEDULE] push stream no received messages -> skip this cycle total_received=%s last_recv=%s connected=%s ws_alive=%s patched=v9",
                total_received,
                last_recv,
                connected,
                ws_alive,
            ...
            return False
        stale_sec = max(15.0, _env_float("TONOSAMA_PUSH_LAST_RECV_MAX_AGE_SEC", 90.0))
        try:
            last_age = (dt.datetime.now() - last_recv.replace(tzinfo=None)).total_seconds()
        except Exception:
            last_age = None
        if last_age is not None and last_age > stale_sec:
            logger.warning(
                "[TONOSAMA ENTRY SCHEDULE] push stream last_recv stale -> skip this cycle total_received=%s last_recv=%s age=%.1fs max_age=%.1fs connected=%s ws_alive=%s patched=v9",
                total_received,
                last_recv,
                last_age,
                stale_sec,
        ...
        # 旧構成や単体テストではstateが無い場合があるため、取得不能時だけfail-open。
        logger.debug("[TONOSAMA ENTRY SCHEDULE] push stream health unavailable", exc_info=True)
        return True
...
def _patched_latest_push_summary_age_sec():
    ...
    for name, df in _candidate_dfs():
        ...
        if best_dt is None or latest > best_dt:
            best_age, best_dt, best_rows, best_src = age, latest, rows, src
    logger.info(
        "[TONOSAMA FRESH SUMMARY WAIT FIX] latest lookup latest=%s age=%s rows=%s source=%s future=%s tol=%.1fs",
        best_dt,
        None if best_age is None else round(float(best_age), 1),
        best_rows,
        best_src,
        _is_future_summary_dt(best_dt),
...
def _patched_wait_fresh_push_summary_before_tonosama() -> bool:
    ...
    if _env_bool("TONOSAMA_SKIP_WAIT_OUTSIDE_MARKET_SESSION", True) and not _is_market_session_now(now):
        logger.info(
            "[TONOSAMA ENTRY SCHEDULE] fresh push summary wait skipped outside market session now=%s patched=v9",
            now.strftime("%Y-%m-%d %H:%M:%S"),
        )
        return False

    if not _push_stream_receive_ok():
        return False
    ...
    while True:
        ...
        if _is_future_summary_dt(latest, now):
            logger.warning(
                "[TONOSAMA ENTRY SCHEDULE] future push summary blocked latest=%s now=%s age=%s rows=%s tolerance=%.1fs patched=v9",
                latest,
                now,
        ...
        if _env_bool("TONOSAMA_SKIP_STALE_DURING_LUNCH_REOPEN", True) and _is_lunch_reopen_grace(now) and _latest_is_before_lunch_reopen(latest, now):
            logger.warning(
                "[TONOSAMA ENTRY SCHEDULE] fresh push summary skip lunch reopen stale latest=%s age=%s rows=%s grace_min=%.1f patched=v9",
                latest,
                None if age is None else round(float(age), 1),
                rows,
        ...
        if age is not None and 0 <= age <= max_age:
            logger.info(
                "[TONOSAMA ENTRY SCHEDULE] fresh push summary ok latest=%s age=%.1fs rows=%s max_age=%.1fs patched=v9",
                latest,
                age,
                rows,
                max_age,
        ...
        if latest is not None and age is not None and age > max_age:
            logger.warning(
                "[TONOSAMA ENTRY SCHEDULE] fresh push summary stale immediate skip latest=%s age=%.1fs rows=%s max_age=%.1fs patched=v9 fail_open_stale=0",
                latest,
                age,
                rows,
                max_age,
        ...
        if latest is None and fail_open_empty:
            logger.warning(
                "[TONOSAMA ENTRY SCHEDULE] fresh push summary unavailable latest=None rows=%s -> fail-open to Tonosama body patched=v9",
                rows,
            )
            return True
        ...
        if time.perf_counter() >= deadline:
            if last_dt is None and fail_open_empty:
                logger.warning(
                    "[TONOSAMA ENTRY SCHEDULE] fresh push summary wait expired latest=None rows=%s -> fail-open to Tonosama body patched=v9",
                    last_rows,
                )
                return True
            logger.warning(
                "[TONOSAMA ENTRY SCHEDULE] fresh push summary wait expired latest=%s age=%s rows=%s max_age=%.1fs wait_sec=%.1fs -> skip this cycle patched=v9",
                last_dt,
...
def _fresh_context_df(max_age: float) -> tuple[Any | None, str, float | None, dt.datetime | None, int]:
    ...
    for name, df in _context_candidate_dfs():
        ...
        if _is_future_summary_dt(latest, now):
            logger.warning(
                "[TONOSAMA SUMMARY LOADER WAIT] future context summary ignored source=%s latest=%s now=%s age=%s rows=%s tolerance=%.1fs",
                name,
                latest,
                now,
                None if age is None else round(float(age), 1),
            ...
            continue
        if age is not None and 0 <= age <= max_age and (best_dt is None or latest > best_dt):
            best = df
            best_name = name
            best_age = age
            best_dt = latest
...
def _patch_tonosama_summary_loader() -> bool:
    ...
    if not callable(old):
        return False
    if getattr(old, "_tonosama_loader_wait_v9", False):
        return True
    ...
    def _patched_load_merged_summary(interval: int):
        ...
        try:
            ...
            if isinstance(df, pd.DataFrame) and not df.empty:
                ...
                if latest is not None and _is_future_summary_dt(latest):
                    logger.warning(
                        "[TONOSAMA SUMMARY LOADER WAIT] original future interval=%s rows=%s latest=%s age=%s source=%s -> wait context",
                        interval_i,
                        rows,
                        latest,
                        None if age is None else round(float(age), 1),
    ...
    _patched_load_merged_summary._tonosama_loader_wait_v9 = True  # type: ignore[attr-defined]
    ...
    logger.warning(
        "[TONOSAMA SUMMARY LOADER WAIT] installed v9 empty_wait_sec=%s poll=%s future_tolerance_sec=%s",
        os.getenv("TONOSAMA_SUMMARY_LOADER_EMPTY_WAIT_SEC", "5.0"),
        os.getenv("TONOSAMA_SUMMARY_LOADER_EMPTY_POLL_SEC", "0.35"),
        os.getenv("TONOSAMA_FUTURE_SUMMARY_TOLERANCE_SEC", "60"),
    )
    return True
...
def _patch_push_monitor_symbol_universe() -> bool:
    ...
    if not callable(old):
        return False
    if getattr(old, "_tonosama_push_universe_restore_v9", False):
        return True
    ...
    _patched_resolve_monitor_symbols._tonosama_push_universe_restore_v9 = True  # type: ignore[attr-defined]
    ...
    logger.warning(
        "[PUSH MONITOR SYMBOL RESTORE] installed v9 min_universe=%s max_symbols=%s",
...
def _apply() -> bool:
    ...
    try:
        ...
        if getattr(cur, "_tonosama_fresh_summary_wait_fix_v9", False):
            _patch_tonosama_summary_loader()
            _patch_push_monitor_symbol_universe()
            _INSTALLED = True
            return True
        _patched_latest_push_summary_age_sec._tonosama_fresh_summary_wait_fix_v9 = True  # type: ignore[attr-defined]
        ...
        _patched_wait_fresh_push_summary_before_tonosama._tonosama_fresh_summary_wait_fix_v9 = True  # type: ignore[attr-defined]
        ...
        os.environ.setdefault("TONOSAMA_FUTURE_SUMMARY_TOLERANCE_SEC", "60")
        os.environ.setdefault("TONOSAMA_REQUIRE_PUSH_STREAM_RECEIVE", "1")
        os.environ.setdefault("TONOSAMA_PUSH_LAST_RECV_MAX_AGE_SEC", "90")
        ...
        _INSTALLED = True
        logger.warning(
            "[TONOSAMA FRESH SUMMARY WAIT FIX] installed v9 future_block=1 push_receive_required=%s loader_wait=%s universe_restore=%s fail_open_empty=%s fail_open_stale=%s skip_wait_outside_session=%s skip_lunch_reopen_stale=%s grace_min=%s future_tol=%s push_last_recv_max_age=%s immediate_stale_skip=1",
            os.environ.get("TONOSAMA_REQUIRE_PUSH_STREAM_RECEIVE"),
            loader_ok,
            universe_ok,
            ...
            os.environ.get("TONOSAMA_FUTURE_SUMMARY_TOLERANCE_SEC"),
            os.environ.get("TONOSAMA_PUSH_LAST_RECV_MAX_AGE_SEC"),
        )
        return True
    except Exception:
