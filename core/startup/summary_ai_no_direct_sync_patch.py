...
def apply_patch() -> bool:
    ...
    # main.py 側でも summary parent tick を必ず戻す。
    # 既に他patch/usercustomizeで 1 が入っていても強制的に 0 に戻す。
    ...
    os.environ['SUMMARY_PUSH_BG_ALL_INTERVALS'] = '1'
    os.environ['SUMMARY_PUSH_BG_LONG_INTERVALS'] = '1'
    os.environ['SUMMARY_PARALLEL_FORCE_1_3_5'] = '1'
...
def watch():
    loops = max(1, min(int(float(os.getenv('SUMMARY_AI_NO_DIRECT_SYNC_WATCH_LOOPS', '90') or 90)), 240))
    sleep_sec = max(0.5, min(float(os.getenv('SUMMARY_AI_NO_DIRECT_SYNC_WATCH_SLEEP_SEC', '1.0') or 1.0), 5.0))
    for i in range(loops):
        ok = apply_patch()
        if i in (0, 1, 5, 15, 30, 60, loops - 1):
            logger.warning(
                '[SUMMARY AI NO DIRECT SYNC] enforce v5 i=%s/%s ok=%s direct_sync=%s direct_dispatch=%s parent_skip=%s force_parent=%s no5s=%s board_adv=%s',
                i, loops, ok,
...
def install() -> bool:
    ...
    _DONE = True
    logger.warning(
        '[SUMMARY AI NO DIRECT SYNC] installed v5 ok=%s watcher=True direct_sync=%s direct_dispatch=%s parent_skip=%s force_parent=%s no5s=%s board_adv=%s',
        ok,
        os.environ.get('SUMMARY_AI_ASYNC_ENTRY_DIRECT_SYNC'),
        os.environ.get('SUMMARY_AI_DIRECT_DISPATCH_ON_QUEUED_ASYNC'),
        os.environ.get('AUTOSTOCK_MAIN_SKIP_SUMMARY_PARENT_TICK'),
