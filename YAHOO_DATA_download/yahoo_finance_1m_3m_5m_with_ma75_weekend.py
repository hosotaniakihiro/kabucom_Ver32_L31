...
def get_latest_trading_days(n=2, base_date=None):
    if base_date is None:
        base_date = dt.date.today()
...
# ============================================================
# Yahoo 1分足（前営業日＋前々営業日）
# ============================================================
def get_1min_data(symbol: str) -> pd.DataFrame:
    trading_days = get_latest_trading_days(2)
    prev_day, prev2_day = trading_days[0], trading_days[1]

    start_dt = dt.datetime.combine(prev2_day, dt.time(0, 0))
    end_dt = dt.datetime.combine(prev_day, dt.time(23, 59))
...
def process_all(excel_path, output_dir):
    ...
    for _, row in df_excel.iterrows():
        ...
        if not df_3m.empty:
            prev_3m = load_prev_nmin(code, output_dir, 3)
            df_3m = add_ma75_with_prev(df_3m, prev_3m)
        ...
        if not df_5m.empty:
            prev_5m = load_prev_nmin(code, output_dir, 5)
