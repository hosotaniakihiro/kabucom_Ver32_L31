...
# 営業日取得（★これ1本に統一）
# ============================================================
def get_latest_trading_days(n=4, base_date=None):
    ...
    return sorted(days)   # ★ 時系列保証


def get_latest_trading_day(base_date=None):
    return get_latest_trading_days(1, base_date)[0]
...
def calc_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(len(df)):
        ...
        rci_vals.append(
            (1 - (6 * (d ** 2).sum()) / (period * (period ** 2 - 1))) * 100
        )
    df["rci"] = rci_vals
...
def get_1min_data(symbol: str) -> pd.DataFrame:
    trading_days = get_latest_trading_days(4)

    start_day = trading_days[0]
    end_day = trading_days[-1]

    start_dt = dt.datetime.combine(start_day, dt.time(0, 0))
    end_dt = dt.datetime.combine(end_day, dt.time(23, 59))
    ...
    try:
        ...
        valid_dates = {d.strftime("%Y-%m-%d") for d in trading_days}
        df["date"] = df["time_range"].dt.strftime("%Y-%m-%d")
...
# 前営業日 N分足ロード（MA75補完）
# ============================================================
def load_prev_nmin(symbol, base_dir, interval, limit=75):
...
def add_ma75_with_prev(df_today, df_prev):
    df = pd.concat([df_prev, df_today], ignore_index=True)
    df["ma75"] = df["close_price"].rolling(75).mean()
    return df.iloc[len(df_prev):].reset_index(drop=True)
...
def process_all(excel_path, output_dir):
    ...
    for _, row in df_excel.iterrows():
        ...
        if not df_3m.empty:
            prev = load_prev_nmin(code, output_dir, 3)
            df_3m = calc_indicators(add_ma75_with_prev(df_3m, prev))
            save_daily_by_interval(df_3m, code, name, output_dir, 3)

        df_5m = resample_nmin(df_1m, 5)
        if not df_5m.empty:
            prev = load_prev_nmin(code, output_dir, 5)
            df_5m = calc_indicators(add_ma75_with_prev(df_5m, prev))
