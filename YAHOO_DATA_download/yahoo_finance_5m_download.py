...
def save_to_sqlite_daily(db_path, df, code, name, table='stock_summary'):
    try:
        ...
        # 1銘柄・1日ごとに削除して入れ替え
        conn.execute(f"DELETE FROM {table} WHERE symbol = ? AND date = ?",
                     (code, df['date'].iloc[0]))
...
def process_and_save_all_stock_data_batch(excel_file_path, output_base_dir, days_to_fetch_api, days_to_load_local):
    ...
    print(f"📅 取得期間：{start_dt.date()} → {end_dt.date()}（{days_to_fetch_api}日間）")
    ...
    for idx, row in df_excel.iterrows():
        ...
        df = calculate_rsi_stoch(df)
        df = calculate_rci(df)
        df = calculate_vwap(df)
        df = calculate_bollinger_bands(df)

        df['time_range'] = pd.to_datetime(df['time_range']).dt.strftime("%Y-%m-%d %H:%M:%S")

        # 日付ごとにDB保存
        for single_date in df['time_range'].str[:10].unique():
            daily = df[pd.to_datetime(df['time_range']).dt.strftime("%Y-%m-%d") == single_date]
            if not daily.empty:
