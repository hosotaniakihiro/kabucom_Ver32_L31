# ============================================================
# File   : database/schema/ranking_summary_schema.py
# Version: PRODUCTION-STABLE-REV1.1-RANKING-SUMMARY-SCHEMA
# ============================================================
...
RANKING_SUMMARY_COLUMNS: List[tuple[str, str]] = [
    ...
    # REV1.1:
    # fallback saver / feature側が atr を保存するため schema 側にも追加
    ("atr", "REAL"),
    ("atr_1m", "REAL"),
...
def table_exists(con: sqlite3.Connection, table: str) -> bool:
    try:
        ...
        logger.exception("[RANKING SUMMARY SCHEMA] table_exists failed table=%s", table)
        return False
...
def index_exists(con: sqlite3.Connection, index_name: str) -> bool:
    try:
        ...
        logger.exception("[RANKING SUMMARY SCHEMA] index_exists failed index=%s", index_name)
        return False
...
def get_existing_columns(con: sqlite3.Connection, table: str) -> Dict[str, Dict[str, Any]]:
    ...
    try:
        ...
        logger.warning(
            "[RANKING SUMMARY SCHEMA] get columns failed table=%s",
            table,
            exc_info=True,
        )
...
def delete_null_key_rows(con: sqlite3.Connection, table: str) -> int:
    try:
        ...
        if deleted > 0:
            logger.warning(
                "[RANKING SUMMARY SCHEMA] null-key rows deleted table=%s before=%s after=%s deleted=%s",
                table,
                before,
                after,
                deleted,
        ...
        logger.warning(
            "[RANKING SUMMARY SCHEMA] null-key cleanup skipped table=%s",
            table,
            exc_info=True,
        )
        return 0
...
def dedupe_ranking_summary_table(con: sqlite3.Connection, table: str) -> int:
    try:
        ...
        if deleted > 0:
            logger.warning(
                "[RANKING SUMMARY SCHEMA] dedupe done table=%s duplicate_keys=%s before=%s after=%s deleted=%s",
                table,
                dup_count,
                before,
                after,
        ...
        logger.exception("[RANKING SUMMARY SCHEMA] dedupe failed table=%s", table)
        return 0
...
def add_missing_columns(con: sqlite3.Connection, table: str) -> None:
    ...
    for col, typ in expected.items():
        ...
        # SQLite は既存テーブルへの NOT NULL without DEFAULT を嫌うため緩和
        ...
        try:
            ...
            added += 1
            logger.warning(
                "[RANKING SUMMARY SCHEMA] added missing column table=%s column=%s type=%s",
                table,
                col,
                add_typ,
            )
            ...
            logger.warning(
                "[RANKING SUMMARY SCHEMA] add column failed table=%s column=%s err=%s",
                table,
                col,
                exc,
                exc_info=True,
    ...
    if added > 0:
        logger.warning(
            "[RANKING SUMMARY SCHEMA] added missing columns table=%s added=%s",
            table,
            added,
        )


def ensure_ranking_summary_table(con: sqlite3.Connection, *, interval: int) -> None:
    ...
    if not index_exists(con, index_name):
        try:
            ...
            logger.info(
                "[RANKING SUMMARY SCHEMA] unique index ensured table=%s index=%s",
                table,
                index_name,
            )
            ...
            logger.warning(
                "[RANKING SUMMARY SCHEMA] unique index retry after cleanup table=%s",
                table,
                exc_info=True,
            )
            ...
            logger.info(
                "[RANKING SUMMARY SCHEMA] unique index ensured after retry table=%s index=%s",
                table,
                index_name,
            )
    ...
    con.execute(
        f"""
        CREATE INDEX IF NOT EXISTS {quote_ident(f"idx_{table}_datetime")}
    ...
    con.execute(
        f"""
        CREATE INDEX IF NOT EXISTS {quote_ident(f"idx_{table}_score")}
    ...
    con.execute(
        f"""
        CREATE INDEX IF NOT EXISTS {quote_ident(f"idx_{table}_symbol_datetime")}
        ON {quote_ident(table)}(symbol, datetime)
        """
    )
    ...
    con.execute(
        f"""
        CREATE INDEX IF NOT EXISTS {quote_ident(f"idx_{table}_ranking_type")}
    ...
    *,
    ...
    for attempt in range(1, MAX_SAVE_RETRY + 1):
        ...
        try:
            ...
            logger.info(
                "[RANKING SUMMARY SCHEMA] all tables ensured path=%s attempt=%s",
                path,
                attempt,
            )
            return
            ...
            if is_lock_error(exc) and attempt < MAX_SAVE_RETRY:
                slept = lock_sleep_seconds(attempt)
                logger.warning(
                    "[RANKING SUMMARY SCHEMA] ensure_all locked retry path=%s attempt=%s/%s sleep=%.2fs err=%s",
                    path,
                    attempt,
                    MAX_SAVE_RETRY,
                    slept,
            ...
            logger.exception("[RANKING SUMMARY SCHEMA] ensure_all failed path=%s", path)
    ...
    logger.error(
        "[RANKING SUMMARY SCHEMA] ensure_all failed after retries path=%s last_err=%s",
        path,
